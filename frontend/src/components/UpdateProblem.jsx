...
function UpdateProblem() {
    ...
    // Fetch and populate form data from admin endpoint
    useEffect(() => {
        async function fetchAndPopulateProblem() {
            try {
                const response = await axiosClient.get(`/problem/admin/${id}`);
                const data = response.data;

                // Directly reset form with fetched data including hiddenTestCases
                reset({
                    title: data.title || '',
                ...
                // If the problem doesn't exist or the user is not admin, redirect to home
        ...
        <div className="min-h-screen bg-linear-to-br from-[#1a1a1a] to-[#202020]">
            <div className="container mx-auto px-4 py-8 max-w-5xl">
                <div className="mb-8 text-center">
                    <h1 className="text-4xl font-bold bg-linear-to-r  bg-clip-text text-white mb-2">
                        Update Problem
                    </h1>
                    <p className="text-gray-400">Edit the coding challenge details</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    {/* Action Buttons */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 pb-8">
                        <button
                            type="button"
                            onClick={() => navigate('/')}
                            className="btn btn-lg w-full bg-[#2A2A2A] hover:bg-[#333] border border-[#444] text-gray-300 font-bold py-4 rounded-xl"
                        >
